...
public class ProjetAllBuildingsElementsPaymentsReport extends javax.swing.JFrame {
...
JFrame frame ;
    ...
    public ProjetAllBuildingsElementsPaymentsReport(javax.swing.JFrame mainFram) throws SQLException {
        frame=this;
        invoke = mainFram;
        ...
        inConn.fillComboBox("workers", "worker_name", ReferenceComboBox);
        this.setTitle("احصائية المستحقات التفصيلية لكل المحاضر للمهني " );
        jLabel2.setText("احصائية المستحقات التفصيلية لكل المحاضر للمهني " );
        /*inputs*/
    ...
    private JPanel showPie1Chart() {
        ...
        try {
            ...
            Logger.getLogger(ProjetAllBuildingsElementsPaymentsReport.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void showSelectedTypeResults() throws SQLException {
         ...
         int worker_id = inConn.getIDByDesc("workers", ReferenceComboBox.getSelectedItem().toString(), "worker_name", "worker_id");

        
        quantityPerReferenceQuery = "SELECT \n"
                + "sum (AA.total_price ) -COALESCE((SELECT  sum(payment_element_records.amount) FROM payment_element_records  \n"
                + " left join  workers on payment_element_records.worker_id = workers.worker_id \n"
                + "  where  payment_element_records.buildno=AA.buildno and workers.worker_id='"+worker_id+"') ,0) as \"باقي المستحقات\",\n"
                + "(SELECT  sum(payment_element_records.amount) FROM payment_element_records  \n"
                + " left join  workers on payment_element_records.worker_id = workers.worker_id \n"
                + "  where payment_element_records.buildno=AA.buildno and workers.worker_id='"+worker_id+"') as \"اجمالي المدفوعات\",\n"
                + " sum (AA.total_price ) as \"اجمالي المستحقات\",\n"
                + "  buildings.desc_ as \"المحضر\"\n"
                + "  FROM rec_elements AA\n"
                + "  left join  workers on AA.worker_id = workers.worker_id \n"
        ...
        inConn.QueryTable(quantityPerReferenceQuery, inputsRecordsjTable);
         ...
         this.setTitle("احصائية المستحقات التفصيلية لكل المحاضر للمهني " +  ReferenceComboBox.getSelectedItem().toString());
        jLabel2.setText("احصائية المستحقات التفصيلية لكل المحاضر للمهني " +  ReferenceComboBox.getSelectedItem().toString());
