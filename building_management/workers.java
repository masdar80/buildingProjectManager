...
public class workers extends javax.swing.JFrame {
    ...
    String[] fields = {"worker_name","worker_add","worker_tel","worker_mob","worker_email","notices"};
    String tableName = "workers";
    String tableKey = "worker_id";
    String query="SELECT "
            + " notices as " +'"'+ "ملاحظات " +'"'
            +", worker_email  as " +'"'+ "الايميل " +'"'
            +", worker_mob  as " +'"'+ "الجوال" +'"'
            +", worker_tel as " +'"'+ "الهاتف" +'"'
            +", worker_add as " +'"'+ "العنوان" +'"'
            +", worker_name  as " +'"'+ "المورد" +'"'
            +" FROM workers  ";
   

    public workers(javax.swing.JFrame mainFram)throws SQLException {
         invoke =mainFram;
      ...
      this.setSize(736, 640);
        this.setLocation((temp_width - 736) / 2, (temp_height - 640) / 2);
    ...
    private void editselectedActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editselectedActionPerformed
        ...
        if(selectedRow<0)
        {
            showMessage("يجب تحديد مهني لتعديله ",false);
                return;
        }
...
//if(Functions.deletePrevilege){
        ...
        try {
            ...
            id = conn.searchIdwithcondition(tableName, tableKey, "worker_name", valueIndex);
        } catch (SQLException ex) {
            Logger.getLogger(workers.class.getName()).log(Level.SEVERE, null, ex);
        }
           try {
        new addworker(this,id).setVisible(true);
    } catch (SQLException ex) {
...
//if(Functions.deletePrevilege){
    ...
    private void DoQueryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_DoQueryActionPerformed
        ...
        if (!Functions.validation(customerName.getText(), 1, true)) {
            ...
            showMessage("يجب ادخال اسمس المهني",false);
        ...
        try {
            if(conn.checkUniqField("worker_name",customerName.getText() , tableName))
            {
               
                showMessage("يوجد مهني بنفس الإسم",false);
                return;
            }
