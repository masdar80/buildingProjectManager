...
public class diging_balances_report extends javax.swing.JFrame {
   ...
   String bldno;
    ...
    public diging_balances_report(javax.swing.JFrame mainFram,String bldno)throws SQLException {
        invoke = mainFram;
        invoke.setEnabled(false);
        inConn = new connect();
        inConn.newConnection();
   this.bldno=bldno;
   quantityPerReferenceQuery = "SELECT ((select sum (rec_diging.total_price) from rec_diging where buildno='"+this.bldno+"'"+" and  rec_diging.diging_id=ref.diging_id) - (select sum (payment_diging_records.amount) from payment_diging_records where buildno='"+this.bldno+"'"+" and  payment_diging_records.diging_id=ref.diging_id)) as \"باقي المستحقات\"," +
"	  (select sum (payment_diging_records.amount) from payment_diging_records where buildno='"+this.bldno+"'"+" and  payment_diging_records.diging_id=ref.diging_id)  as \"قيمة المدفوعات\", " +
"            (select sum (rec_diging.total_price) from rec_diging where buildno='"+this.bldno+"'"+" and  rec_diging.diging_id=ref.diging_id) as \"قيمة الحفريات\"," +
"	     ref.unit  as \"الواحدة\"," +
...
"           FROM diging_ ref";
        ...
        this.setIconImage(img);   
         ...
         this.setTitle("نقرير باقي مستحقات الحفريات  للمحضر رقم  "+bldno);
         jLabel2.setText("نقرير باقي مستحقات الحفريات  للمحضر رقم  "+bldno);
 ...
 private JPanel showChartPrice() {
        ...
        String chartQuantityPerReferenceQuery = "SELECT ((select sum (rec_diging.total_price) from rec_diging where buildno='"+this.bldno+"'"+" and  rec_diging.diging_id=ref.diging_id) - (select sum (payment_diging_records.amount) from payment_diging_records where buildno='"+this.bldno+"'"+" and  payment_diging_records.diging_id=ref.diging_id)) as result,\n" +
"	  (select sum (payment_diging_records.amount) from payment_diging_records where buildno='"+this.bldno+"'"+" and  payment_diging_records.diging_id=ref.diging_id)  as payments, \n" +
"            (select sum (rec_diging.total_price) from rec_diging where buildno='"+this.bldno+"'"+" and  rec_diging.diging_id=ref.diging_id) as prices ,\n" +
"	     ref.unit  ,\n" +
"            ref.desc_  \n" +
"           FROM diging_ ref";
        try {
            ...
            Logger.getLogger(diging_balances_report.class.getName()).log(Level.SEVERE, null, ex);
        }

        return Functions.createPieChartPanel(" باقي مستحقات الحفريات  للمحضر رقم  "+bldno, pieDataset,this.getWidth()/2,300);
    }
...
"           FROM diging_ ref";
        ...
        return Functions.createBarChartPanel(" باقي مستحقات الحفريات  للمحضر رقم  "+bldno,"الحفرياتس","المستحقات", barDataset,this.getWidth()/2,300);
    }
    ...
    private void showAllSumResults() {
        ...
        Statement stmt = null;
        DefaultPieDataset pieDataset = new DefaultPieDataset();
        String chartQuantityPerReferenceQuery = "SELECT sum(((select sum (rec_diging.total_price) from rec_diging where buildno='"+this.bldno+"'"+"  ) - (select sum (payment_diging_records.amount) from payment_diging_records where buildno='"+this.bldno+"'"+" )) )as result,\n" +
"	  sum((select sum (payment_diging_records.amount) from payment_diging_records where buildno='"+this.bldno+"'"+" ))  as payments, \n" +
"           sum( (select sum (rec_diging.total_price) from rec_diging where buildno='"+this.bldno+"'"+" )) as prices ";
        try {
            stmt = inConn.conn.createStatement();
            ResultSet res = stmt.executeQuery(chartQuantityPerReferenceQuery);
            while (res.next()) {
