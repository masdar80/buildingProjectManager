...
public class element_balances_report extends javax.swing.JFrame {
   ...
   String bldno,bld_desc;
    ...
    public element_balances_report(javax.swing.JFrame mainFram,String bldno,String bld_desc)throws SQLException {
        invoke = mainFram;
   ...
   quantityPerReferenceQuery = "SELECT \"باقي المستحقات\", \"قيمة المدفوعات\", \"قيمة المستحقات\", \"المهني\"\n" +
"  FROM element_smmary_one_building;";
        initComponents();
        ...
        this.setIconImage(img);   
         ...
         this.setTitle(" باقي مستحقات المهنيين  للمحضر  "+bld_desc);
         jLabel2.setText(" باقي مستحقات المهنيين  للمحضر  "+bld_desc);
 ...
 private JPanel showChartPrice() {
        ...
        try {
            ...
            while (res.next()) {
                ...
                pieDataset.setValue(res.getString(4) + " " + res.getDouble(1), new Double(res.getDouble(1)));
            }

        } catch (SQLException ex) {
            Logger.getLogger(element_balances_report.class.getName()).log(Level.SEVERE, null, ex);
        }

        return Functions.createPieChartPanel(" باقي مستحقات المهنيين  للمحضر رقم  "+bldno, pieDataset,this.getWidth()/2,300);
    }
       ...
       private JPanel showBarChartQuantity() {
        ...
        return Functions.createBarChartPanel(" باقي مستحقات المهنيين  للمحضر رقم  "+bldno,"المهني","القيم", barDataset,this.getWidth()/2,300);
    }
    ...
    private void showAllSumResults() {
        ...
        Statement stmt = null;
        DefaultPieDataset pieDataset = new DefaultPieDataset();
        String chartQuantityPerReferenceQuery = "SELECT sum(((select sum (rec_elements.total_price) from rec_elements where buildno='"+this.bldno+"'"+" ) - (select sum (payment_element_records.amount) from payment_element_records where buildno='"+this.bldno+"'"+" )) )as result,\n" +
"	  sum((select sum (payment_element_records.amount) from payment_element_records where buildno='"+this.bldno+"'"+" ))  as payments, \n" +
"           sum( (select sum (rec_elements.total_price) from rec_elements where buildno='"+this.bldno+"'"+" )) as prices ";
        try {
            stmt = inConn.conn.createStatement();
