...
public class refPaymentReport extends javax.swing.JFrame {
    ...
    public refPaymentReport(javax.swing.JFrame mainFram, String bldno, String bld_desc, String refrence_type_id, String refrence_type_dsec) throws SQLException {
        invoke = mainFram;
        ...
        quantityPerReferenceQuery = "select\n"
                + "(sum(AA .total_price) - COALESCE(( select sum(payments_records.amount) \n"
                ...
                + " where references_categories.reference_id='" + this.refrence_type_id + "' \n"
                + "  and payments_records.buildno='" + this.bldno + "'\n"
                + "   and  vendors.vendor_id= AA.vendor_id),0)) as \"باقي المستحقات \",\n"
                + "( select sum(payments_records.amount) \n"
                + "FROM payments_records    \n"
                + "left join  references_categories on payments_records.category_id = references_categories.reference_id  \n"
                + "left join  vendors on payments_records.vendor_id = vendors.vendor_id \n"
                + " where references_categories.reference_id='" + this.refrence_type_id + "' \n"
                + "  and payments_records.buildno='" + this.bldno + "'\n"
                + "   and  vendors.vendor_id= AA.vendor_id) as \"مجموع المدفوعات \",  \n"
                + "sum(AA .total_price) as \"مجموع المستحقات \",\n"
                ...
                + " and  references_categories.reference_id='" + this.refrence_type_id + "' \n"
        ...
        this.setTitle(" محصلة الموردين للاحضار نوع " + this.refrence_type_dsec + " للمحضر " + bld_desc);
        jLabel2.setText(" محصلة الموردين للاحضار نوع " + this.refrence_type_dsec + " للمحضر " + bld_desc);
        /*inputs*/
    ...
    private JPanel showPie1Chart() {
        ...
        try {
            ...
            Logger.getLogger(refPaymentReport.class.getName()).log(Level.SEVERE, null, ex);
        }
