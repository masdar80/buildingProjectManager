...
public class digingHistoricalReport extends javax.swing.JFrame {
...
JFrame frame ;
    ...
    public digingHistoricalReport(javax.swing.JFrame mainFram, String bldno) throws SQLException {
        invoke = mainFram;
 ...
 frame = this;
        ...
        conn.newConnection();
         this.bldno = bldno;
         quantityPerReferenceQuery = "SELECT sum (rec_diging.total_price ) as \"اجمالي المبالغ\","
            ...
            + "group by    date_,diging_.desc_,diging_.unit   order by rec_diging.date_";
        initComponents();
        ...
        this.setTitle("نقرير عمل حفريات محدد للمحضر رقم  " + bldno);
        jLabel2.setText("نقرير عمل حفريات محدد للمحضر رقم  " + bldno);
        /*inputs*/
        conn.fillComboBox("diging_", "desc_", ReferenceComboBox);
    ...
    private void initComponents() {
        ...
        jLabel5.setText("تحديد بيان العمل :");
    ...
    private JPanel showBarChartQuantity() {
        ...
        int type_id;
        try {
            type_id = conn.getIDByDesc("diging_", ReferenceComboBox.getSelectedItem().toString(), "desc_", "diging_id");

            Statement stmt = null;
            DefaultCategoryDataset barDataset = new DefaultCategoryDataset();
            String chartQuantityPerReferenceQuery = "SELECT sum (rec_diging.total_price ) price,"
                    ...
                    + " FROM rec_diging "
                    + "left join  diging_ on rec_diging.diging_id = diging_.diging_id "
                    + " where rec_diging.diging_id= " + type_id
            ...
            while (res.next()) {
                ...
                barDataset.setValue(new Double(res.getDouble("quantity")), res.getDate("date_"), "("+res.getDouble("price")+")-"+res.getDate("date_"));
...
//            while (res.next()) {
//
//                pieDataset.setValue(res.getString("date_") +"("+res.getInt("price")+")", new Double(res.getDouble("price")));
//            }
//
//        } catch (SQLException ex) {
...
//        }
//
//        return Functions.createPieChartPanel("  مبالغ الاحضارات  للمحضر رقم  "+bldno, pieDataset,this.getWidth()/2,300);
//    }
//    
    
    private void showSelectedTypeResults() throws SQLException {

        int type_id = conn.getIDByDesc("diging_", ReferenceComboBox.getSelectedItem().toString(), "desc_", "diging_id");
        ...
        quantityPerReferenceQuery = "SELECT sum (rec_diging.total_price ) as \"اجمالي المبالغ\","
                ...
                + " where rec_diging.diging_id= " + type_id
                + " and  buildno='"+this.bldno+"'"
                + "group by    date_,diging_.desc_,diging_.unit ";
